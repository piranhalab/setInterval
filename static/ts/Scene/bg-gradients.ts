...
export const bgGradient:bgGradient = {
	...
	init:function(){
		...
		window.addEventListener("changeUser",function(data:CustomEvent){
			let uuid = data.detail.uuid
			let prop = data.detail.prop
			let value = data.detail.value

			if(Users[uuid].uuid != "01800-api-666") return
			if(prop == "bg"){
				if(bgGradient.grads.hasOwnProperty(value)){
					(document.body as HTMLElement).style.backgroundImage = bgGradient.grads[value] 
				}
			}
		})
		
		window.addEventListener("addUser",function(data:CustomEvent){
			let uuid = data.detail.uuid
